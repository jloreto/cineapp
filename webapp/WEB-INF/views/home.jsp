    ...
    ================================================== -->
      ...
      <div class="row page-header">          
        <div class="col-lg-12">         
          ...
          <form class="form-inline" action="${baseUrl}search" method="post">
            <div class="form-group">
              <label for="date">Date: </label>
              <select id="date" name="date" class="form-control">
              	
              	<c:forEach items="${nextDays}" var="nDay">
              		
              	<c:choose>
              		<c:when test="${nDay eq searchDate}">
              			<option value="${nDay}" selected>${nDay}</option>
              		</c:when>
              		<c:otherwise>
      ...
      <div class="container marketing">
		...
		<c:forEach items="${movies}" var="mov">
          <div class="col-xs-12 col-sm-6 col-md-3">
            <img class="img-rounded" src="${publicUrl}/images/${mov.image}" alt="Generic placeholder image" width="150" height="200">
            <h4>${mov.title}</h4>
            <h4>
              <span class="label label-default">${mov.classification}</span>
              <span class="label label-default">${mov.duration} min</span>
              <span class="label label-default">${mov.genre}</span>
            </h4>         
            <p><a class="btn btn-sm btn-primary" href="detail/${mov.id}/${searchDate}" role="button">Consulta Horarios &raquo;</a></p>
            <p><a class="btn btn-sm btn-primary" href="detail/?movieId=${mov.id}" role="button">Consulta Horarios &raquo;</a></p>
          </div>
		</c:forEach>
